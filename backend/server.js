...
app.use('/api', (req, res, next) => {
  if (mongoose.connection.readyState !== 1) {
    return res.status(503).json({
      message: 'Database not connected. Please verify your MONGODB_URI in backend/.env and ensure MongoDB is running.',
    });
  }
  next();
});
...
app.get('/', (req, res) => res.json({ 
  message: 'VaadaTrack API Running',
  database: mongoose.connection.readyState === 1 ? 'Connected' : 'Disconnected (check MONGODB_URI in backend/.env)'
}));
